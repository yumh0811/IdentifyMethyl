...
IdentifyMethyl <- function(x, minCor = .2) {
  ...
  if (length(gkeep) < 0.8 * nrow(centroids)) warning("Input methylation profiles include less than 80% of the CpGs used for classification. Results may not be relevant")

  col_N <- colnames(x)
  x <- sweep(as.data.frame(x[gkeep, ]), 1, as.numeric(medians[gkeep]), FUN = "-")
  colnames(x) <- col_N
  rownames(x) <- gkeep
  cor.dat <- as.data.frame(cor(x, centroids[match(gkeep, rownames(centroids)), classes], use = "complete.obs", method = "pearson"), row.names = colnames(x))
  ...
  cor.dat$cor_pval <- sapply(colnames(x), function(smp) {
    cor.test(x[gkeep, smp], centroids[match(gkeep, rownames(centroids)), cor.dat[smp, "nearestCentroid"]])$p.value
  })
